...
describe('DAO', function () {
  ...
  describe('execute:', async () => {
    ...
    it('emits an event afterwards', async () => {
      let rc;
      try {
        rc = await logGasUsage(
          'DAO.execute() (succeedAction)',
          dao.execute(ZERO_BYTES32, [data.succeedAction], 0)
        );
      } catch (err) {
        console.warn('Gas logging skipped: ', err.message);
        // fallback without logging
        rc = await (await dao.execute(ZERO_BYTES32, [data.succeedAction], 0)).wait();
      }
